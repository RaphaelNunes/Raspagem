<?php
    ...
    //selecionaCandidatos("2012","Acrelândia","AC","11","Eleito");
    leDiretorio($caminhoDir);
    //leArquivoESalvaNoBanco();
    ...
    function leArquivoESalvaNoBanco($caminhoArq){
        ...
        while (($data = fgetcsv($handle, 0, ";")) !== FALSE) {
            ...
            if($estado == "AC")
                echo 'UF:'.$estado.'-Muni:'.$cidade.'-Nr:'.$numero_urna.'-Resultado'.$resultado.'|</br>';
            if($estado == "AC")
...
select ?id ?cidade ?estado ?cargo ?numero {  
  ?id foaf:name ?nome.
  ?id polbr:election ?eleicao.
  ?eleicao timeline:atYear "2012".
  ?eleicao geospecies:City ?cidade.
  filter (regex(?cidade,"Acrelândia","i") ).
  ?eleicao geospecies:State ?estado.
  ?eleicao biblio:number ?numero.
  ?eleicao pol:Office ?cargo.
  filter(?id = <http://ligadonospoliticos.com.br/politico/22647>)
           			}
  insert {
  ?eleicao polbr:situation "Eleito"
            }
where{
  <http://ligadonospoliticos.com.br/politico/22647> polbr:election ?eleicao.
  ?eleicao timeline:atYear "2012".
  ?eleicao geospecies:City ?cidade.
  filter (regex(?cidade,"Acrelândia","i") ).
  }                              
